...
def test_simulated_multiprocess():
    ...
    assert len(scheduler1.get_tasks()) == 1
    assert len(scheduler2.get_tasks()) == 1
    assert dummy_job_called == 0
    with scheduler1, scheduler2:
        assert scheduler1.running
        assert scheduler2.running
        scheduler1.add_task(dummy_job, id="dummy2", replace_existing=True)
        time.sleep(0.1)
        scheduler2.add_task(dummy_job, id="dummy2", replace_existing=True)
