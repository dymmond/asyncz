...
class RedisStore(BaseStore):
    ...
    def get_due_tasks(self, now: datetime) -> list["TaskType"]:
        ...
        ids = cast(list[str], self.redis.zrangebyscore(self.run_times_key, 0, timestamp))
        if not ids:
            return []
        states = cast(list[Any], self.redis.hmget(self.tasks_key, ids))
    ...
    def get_all_tasks(self) -> list["TaskType"]:
        states = cast(list[tuple[str, Any]], self.redis.hgetall(self.tasks_key))
