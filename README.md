...
- CLI commands for starting schedulers, adding tasks, inspecting one task, previewing run times, triggering runs, and managing persisted tasks.
...
- a CLI for `start`, `add`, `list`, `inspect`, `preview`, `status`, `run`, `pause`, `resume`, and `remove`
